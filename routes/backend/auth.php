...
use App\Domains\Auth\Http\Controllers\Backend\Loan\LoanController;
...
Route::group([
    ...
    Route::group([
        ...
        Route::get('/', [LoanController::class, 'index'])
            ...
            ->breadcrumbs(function (Trail $trail) {
                $trail->parent('admin.dashboard')
                    ->push(__('Loans Management'), route('admin.auth.loan.index'));
            });

        Route::get('view/{loan}', [LoanController::class, 'show'])
            ->name('show')
            ->breadcrumbs(function (Trail $trail, Loan $loan) {
                $trail->parent('admin.dashboard')
                    ->push(__('Viewing loan ID :id', ['id' => $loan->id]), route('admin.auth.loan.show', $loan));
            });

        Route::patch('/', [LoanController::class, 'update'])->name('update');

        Route::get('approve', [LoanController::class, 'approve'])
            ->name('approve')
            ->breadcrumbs(function (Trail $trail) {
                $trail->parent('admin.auth.loan.index')
                    ->push(__('Approve'), route('admin.auth.loan.approve'));
            });
    });
